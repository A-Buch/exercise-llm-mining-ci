...
class Gazetteer:
    ...
    def nominatim_query(self, location:str) -> dict:
        ...
        url_metainfo += '&polygon_geojson=1&addressdetails=1&namedetails=1&accept-language=en'
            
        # API call
        r_metainfo = self.session.get(url_metainfo, timeout=20, headers={'User-agent':None}) 
        
        # Example: check if the API request was successful
        assert r_metainfo.status_code == 200, f"Unexpected status code: {r_metainfo.status_code}"
